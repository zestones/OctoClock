...
import { MessagingPort } from '../../src/ports/messaging.port.js';
...
describe('TimerService.syncComment — delegation', () => {
    ...
    it('delegates to the messaging port with the correct arguments', async () => {
        ...
        expect(fake.syncComment).toHaveBeenCalledWith('https://github.com/owner/repo/issues/1', 'owner', 'repo', 1);
    });

    it('returns the value resolved by the port', async () => {
        const result = await TimerService.syncComment('https://github.com/owner/repo/issues/2', 'owner', 'repo', 2);
...
describe('NoOpMessagingAdapter — timerStarted / timerStopped do not throw', () => {
    ...
    it('syncComment resolves without network call', async () => {
        const result = await adapter.syncComment('https://github.com/owner/repo/issues/1', 'owner', 'repo', 1);
