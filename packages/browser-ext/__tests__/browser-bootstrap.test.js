...
describe('browser-bootstrap', () => {
    it('registers the Chrome storage adapter — StorageService.get routes to chrome.storage.local.get', async () => {
        const { StorageService } = await import('../../core/src/services/storage.service.js');
    ...
    it('without bootstrap the adapter guard throws — proves registration is mandatory', async () => {
        ...
        await expect(StorageService.get('any-key')).rejects.toThrow('StorageService: no adapter registered');

        // Restore so later tests in this file are unaffected
        const { ChromeStorageAdapter } = await import('../src/adapters/chrome-storage.adapter.js');
        const { storageEvents } = await import('../../core/src/services/storage-events.js');
